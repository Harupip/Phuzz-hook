...
The output of the HARgen tool are a set of JSON-based configuration files that can be passed to PHUZZ.
...
The `fuzzer/automated_logins/` folder can contain python scripts that are used to perform initial interactions with the target application before the actual fuzzing process begins, e.g. to obtain a valid session or configure the application to reach a certain state.
...
The fuzzed web application has to run in the `web` container. See `./fuzzer/` for how to add a new web application. The checked-in repo is now WordPress-only by default and ships with WordPress plus example plugin configs.
...
Next, start the `web` container to bring up WordPress with `docker-compose up web --build --force-recreate`. Wait for the initialization of the web server as indicated by `web-1  | [Sun Apr 07 11:15:19.602559 2024] [core:notice] [pid 28] AH00094: Command line: '/usr/sbin/apache2 -D FOREGROUND'`
