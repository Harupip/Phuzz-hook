...
This repository now ships as a slimmed-down WordPress-first PHUZZ workspace. The default checked-in workflow focuses on fuzzing WordPress plugins with PHUZZ, while the research paper context remains available as historical reference.

To learn how to use the WordPress-focused setup, please check the README in `./code/`.
