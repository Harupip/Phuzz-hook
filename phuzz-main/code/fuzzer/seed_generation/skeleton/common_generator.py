"""Shared seed-template/report mechanics for explicit evidence-source generators."""
...
from discovery.entrypoints.entrypoints import direct_http_details, seed_template_for_callback
...
class SeedGeneratorBase:
    ...
    def _extract_input_params(self, registered_entry: dict[str, Any]) -> dict[str, Any]:
        raise NotImplementedError("Seed generator must define its parameter evidence source")
    ...
    def _generate_seed_templates(
        ...
        variants = []
        decisions = self._method_decisions(hook_name, callback_metadata or {}, input_params or [])
