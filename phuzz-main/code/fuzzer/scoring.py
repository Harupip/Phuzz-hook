...
def calculate_hook_coverage_energy(request_data, state=None, config=None, update_state=False):
    from hook_energy import EnergyCalculator

    calculator = EnergyCalculator(config=config)
    if state is not None:
        calculator.state = state

    if update_state:
        return calculator.process_request(request_data)
    return calculator.calculate(request_data)
