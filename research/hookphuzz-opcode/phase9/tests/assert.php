...
if ($mode === 'discovery') {
    ...
    foreach (['hookphuzz_phase9_function_probe', 'HookPhuzz_Phase8_Handler::probe', 'HookPhuzz_Phase8_Handler_A::probe', 'HookPhuzz_Phase8_Handler_B::probe'] as $name) if (!in_array($name, $names, true)) fail("missing $name");
...
if ($mode === 'function') {
    $a = artifact($argv[2]); $events = root_events($a, 'hookphuzz_phase9_function_probe');
...
if ($mode === 'cleanup') { $a=artifact($argv[2]); $events=root_events($a,'hookphuzz_phase9_function_probe'); require_event($events,'POST',['after_catch'],0); report(['request_id'=>$a['request_id']]); }
