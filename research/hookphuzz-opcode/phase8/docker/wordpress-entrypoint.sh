...
plugin_path="$wp_path/wp-content/plugins/hookphuzz-phase8-fixture"
mu_plugin_path="$wp_path/wp-content/mu-plugins/hookphuzz-phase8-discovery.php"
...
cp /opt/phase8-instrumentation/hookphuzz-phase8-discovery.php "$mu_plugin_path"
chown www-data:www-data "$mu_plugin_path"

if ! wp core is-installed "${wp_args[@]}"; then
  wp core install "${wp_args[@]}" --url="$WORDPRESS_URL" --title='HookPhuzz Phase 8' \
    --admin_user=phase8admin --admin_password=phase8admin --admin_email=phase8@example.test --skip-email
fi

rm -rf "$plugin_path"
mkdir -p "$(dirname "$plugin_path")"
cp -a /opt/phase8-plugin "$plugin_path"
chown -R www-data:www-data "$plugin_path"
wp plugin activate hookphuzz-phase8-fixture "${wp_args[@]}"
