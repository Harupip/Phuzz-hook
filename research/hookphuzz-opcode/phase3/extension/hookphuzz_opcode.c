...
static void hookphuzz_release_path(hookphuzz_path_key *path, uint32_t depth)
{
    ...
    for (index = 0; index < depth; index++) {
        if (path[index].string_value != NULL) {
            zend_string_release(path[index].string_value);
        }
    }
    efree(path);
}
...
static hookphuzz_path_key *hookphuzz_copy_path(const hookphuzz_path_key *path, uint32_t depth)
{
    ...
    for (index = 0; index < depth; index++) {
        ...
        if (path[index].string_value != NULL) {
            copy[index].string_value = zend_string_copy(path[index].string_value);
        }
    }
    return copy;
}
...
static hookphuzz_path_key *hookphuzz_append_path(const hookphuzz_provenance *provenance, const zval *key)
{
    ...
    for (index = 0; index < old_depth; index++) {
        ...
        if (provenance->path[index].string_value != NULL) {
            path[index].string_value = zend_string_copy(provenance->path[index].string_value);
        }
    }
    path[old_depth].type = Z_TYPE_P(key);
    if (Z_TYPE_P(key) == IS_STRING) {
        path[old_depth].string_value = zend_string_copy(Z_STR_P(key));
    } else {
        path[old_depth].int_value = Z_LVAL_P(key);
    }
    return path;
}
    ...
    hookphuzz_source source, hookphuzz_path_key *path, uint32_t depth)
{
    ...
    if (item == NULL) {
        ...
        if (HOOKPHUZZ_OPCODE_G(provenance) == NULL) {
            HOOKPHUZZ_OPCODE_G(provenance) = ecalloc(HOOKPHUZZ_OPCODE_MAX_EVENTS,
                sizeof(hookphuzz_provenance));
        }
...
static zend_bool hookphuzz_source_from_fetch_r(const zend_execute_data *execute_data,
...
static void hookphuzz_record_event(const zend_execute_data *execute_data, const zend_op *opline,
    const hookphuzz_provenance *provenance, const zval *key, hookphuzz_path_key *path, zend_bool mapped)
{
...
static int hookphuzz_fetch_r_handler(zend_execute_data *execute_data)
{
    hookphuzz_source source;
    const zend_op *opline = execute_data->opline;

    if (hookphuzz_source_from_fetch_r(execute_data, opline, &source)) {
...
static int hookphuzz_fetch_dim_r_handler(zend_execute_data *execute_data)
{
    ...
    if (key == NULL || (Z_TYPE_P(key) != IS_STRING && Z_TYPE_P(key) != IS_LONG)) {
        hookphuzz_record_event(execute_data, opline, provenance, key, NULL, 0);
        return ZEND_USER_OPCODE_DISPATCH;
    }
    path = hookphuzz_append_path(provenance, key);
    hookphuzz_record_event(execute_data, opline, provenance, key,
        hookphuzz_copy_path(path, provenance->depth + 1), 1);
...
{
    if (zend_get_user_opcode_handler(ZEND_FETCH_R) != NULL || zend_get_user_opcode_handler(ZEND_FETCH_DIM_R) != NULL) return FAILURE;
    if (zend_set_user_opcode_handler(ZEND_FETCH_R, hookphuzz_fetch_r_handler) != SUCCESS
        || zend_set_user_opcode_handler(ZEND_FETCH_DIM_R, hookphuzz_fetch_dim_r_handler) != SUCCESS) return FAILURE;
    return SUCCESS;
}

PHP_MSHUTDOWN_FUNCTION(hookphuzz_opcode)
{
    if (zend_get_user_opcode_handler(ZEND_FETCH_R) == hookphuzz_fetch_r_handler) zend_set_user_opcode_handler(ZEND_FETCH_R, NULL);
...
PHP_MINFO_FUNCTION(hookphuzz_opcode)
{
    ...
    php_info_print_table_row(2, "configured user opcodes", "ZEND_FETCH_R, ZEND_FETCH_DIM_R");
