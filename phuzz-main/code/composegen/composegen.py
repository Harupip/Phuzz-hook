...
def get_template(application_type, coverage_path, wp_target_plugin=None) -> str:
    wordpress_env = ""
    if application_type == "wordpress" and wp_target_plugin:
        wordpress_env = f"\n      WP_TARGET_PLUGIN: {wp_target_plugin}"
...
def main():
    ...
    wp_target_plugin = None
    coverage_path = args.coverage_path
    if args.application_type == "wordpress" and configs:
        wp_target_plugin = configs[0]["config_name"].split("/")[-1]
        if not coverage_path:
            coverage_path = f"wp-content/plugins/{wp_target_plugin}/"
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            get_template(args.application_type, coverage_path, wp_target_plugin)
