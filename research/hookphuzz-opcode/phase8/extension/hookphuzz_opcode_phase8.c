...
#include "php.h"
#include "php_hookphuzz_opcode_phase8.h"
...
ZEND_DECLARE_MODULE_GLOBALS(hookphuzz_opcode_phase8)

PHP_INI_BEGIN()
    STD_PHP_INI_ENTRY("hookphuzz_opcode.target_callbacks", "", PHP_INI_SYSTEM, OnUpdateString,
        target_callbacks_ini, zend_hookphuzz_opcode_phase8_globals, hookphuzz_opcode_phase8_globals)
    STD_PHP_INI_ENTRY("hookphuzz_opcode.target_callbacks_file", "", PHP_INI_SYSTEM, OnUpdateString,
        target_callbacks_file_ini, zend_hookphuzz_opcode_phase8_globals, hookphuzz_opcode_phase8_globals)
PHP_INI_END()
...
static void hookphuzz_release_targets(void)
{
    ...
    for (index = 0; index < HOOKPHUZZ_PHASE8_G(target_callback_count); index++) {
        zend_string_release(HOOKPHUZZ_PHASE8_G(target_callbacks)[index]);
    }
    if (HOOKPHUZZ_PHASE8_G(target_callbacks) != NULL) efree(HOOKPHUZZ_PHASE8_G(target_callbacks));
    HOOKPHUZZ_PHASE8_G(target_callbacks) = NULL;
    HOOKPHUZZ_PHASE8_G(target_callback_count) = 0;
    if (HOOKPHUZZ_PHASE8_G(target_load_status) != NULL) zend_string_release(HOOKPHUZZ_PHASE8_G(target_load_status));
    HOOKPHUZZ_PHASE8_G(target_load_status) = NULL;
}

static void hookphuzz_set_target_status(const char *status)
{
    if (HOOKPHUZZ_PHASE8_G(target_load_status) != NULL) zend_string_release(HOOKPHUZZ_PHASE8_G(target_load_status));
    HOOKPHUZZ_PHASE8_G(target_load_status) = zend_string_init(status, strlen(status), 0);
}
...
static zend_bool hookphuzz_valid_callback_name(const zend_string *name)
{
    ...
    if (name == NULL || ZSTR_LEN(name) == 0 || ZSTR_LEN(name) > HOOKPHUZZ_PHASE8_MAX_CALLBACK_BYTES) return 0;
    value = ZSTR_VAL(name);
...
static zend_bool hookphuzz_add_target(const zend_string *name)
{
    ...
    for (index = 0; index < HOOKPHUZZ_PHASE8_G(target_callback_count); index++) {
        if (zend_string_equals_ci(name, HOOKPHUZZ_PHASE8_G(target_callbacks)[index])) return 0;
    }
    if (HOOKPHUZZ_PHASE8_G(target_callback_count) == HOOKPHUZZ_PHASE8_MAX_TARGETS) return 0;
    HOOKPHUZZ_PHASE8_G(target_callbacks) = erealloc(HOOKPHUZZ_PHASE8_G(target_callbacks),
        sizeof(zend_string *) * (HOOKPHUZZ_PHASE8_G(target_callback_count) + 1));
    HOOKPHUZZ_PHASE8_G(target_callbacks)[HOOKPHUZZ_PHASE8_G(target_callback_count)++] = zend_string_copy((zend_string *) name);
    return 1;
}

static void hookphuzz_parse_targets(void)
{
    const char *cursor = HOOKPHUZZ_PHASE8_G(target_callbacks_ini);
    ...
    while (*cursor != '\0') {
        ...
        if (end > start) {
            ...
            if (hookphuzz_add_target(name)) HOOKPHUZZ_PHASE8_G(static_target_count)++;
...
static void hookphuzz_load_file_targets(void)
{
    ...
    if (HOOKPHUZZ_PHASE8_G(target_callbacks_file_ini) == NULL || HOOKPHUZZ_PHASE8_G(target_callbacks_file_ini)[0] == '\0') {
        hookphuzz_set_target_status("disabled");
        return;
    }
    fd = open(HOOKPHUZZ_PHASE8_G(target_callbacks_file_ini), O_RDONLY);
    if (fd < 0) {
        hookphuzz_set_target_status(errno == ENOENT ? "missing" : "malformed");
        return;
    }
    if (fstat(fd, &file_stat) != 0 || file_stat.st_size < 0 || file_stat.st_size > HOOKPHUZZ_PHASE8_MAX_REGISTRY_BYTES) {
    ...
    HOOKPHUZZ_PHASE8_G(registry_schema_version) = 1;
    ZEND_HASH_FOREACH_VAL(registrations_hash, registration) {
        ...
        if (registration_hash == NULL) { HOOKPHUZZ_PHASE8_G(target_rejected_count)++; continue; }
        ...
        if (canonical == NULL || Z_TYPE_P(canonical) != IS_STRING || type == NULL || Z_TYPE_P(type) != IS_STRING
            || (callback != NULL && (Z_TYPE_P(callback) != IS_STRING || Z_STRLEN_P(callback) == 0
                || Z_STRLEN_P(callback) > HOOKPHUZZ_PHASE8_MAX_CALLBACK_BYTES))
            || (strcmp(Z_STRVAL_P(type), "function") != 0 && strcmp(Z_STRVAL_P(type), "static_method") != 0
                && strcmp(Z_STRVAL_P(type), "object_method") != 0)) {
            HOOKPHUZZ_PHASE8_G(target_rejected_count)++; continue;
        }
        if (hookphuzz_add_target(Z_STR_P(canonical))) HOOKPHUZZ_PHASE8_G(file_target_count)++;
        else if (hookphuzz_valid_callback_name(Z_STR_P(canonical))) HOOKPHUZZ_PHASE8_G(target_duplicate_count)++;
        else HOOKPHUZZ_PHASE8_G(target_rejected_count)++;
    } ZEND_HASH_FOREACH_END();
    zval_ptr_dtor(&document);
    hookphuzz_set_target_status((HOOKPHUZZ_PHASE8_G(target_duplicate_count) || HOOKPHUZZ_PHASE8_G(target_rejected_count))
...
static zend_bool hookphuzz_is_target(const zend_string *name)
{
    ...
    for (index = 0; index < HOOKPHUZZ_PHASE8_G(target_callback_count); index++) {
        if (zend_string_equals_ci(name, HOOKPHUZZ_PHASE8_G(target_callbacks)[index])) return 1;
    }
    return 0;
}
...
static hookphuzz_context_frame *hookphuzz_find_context(const zend_execute_data *execute_data)
{
    ...
    for (index = HOOKPHUZZ_PHASE8_G(context_count); index > 0; index--) {
        hookphuzz_context_frame *frame = &HOOKPHUZZ_PHASE8_G(contexts)[index - 1];
...
static void hookphuzz_release_contexts(void)
{
    ...
    for (index = 0; index < HOOKPHUZZ_PHASE8_G(context_count); index++) {
        zend_string_release(HOOKPHUZZ_PHASE8_G(contexts)[index].root_callback);
        zend_string_release(HOOKPHUZZ_PHASE8_G(contexts)[index].current_function);
    }
    if (HOOKPHUZZ_PHASE8_G(contexts) != NULL) efree(HOOKPHUZZ_PHASE8_G(contexts));
    HOOKPHUZZ_PHASE8_G(contexts) = NULL;
    HOOKPHUZZ_PHASE8_G(context_count) = 0;
}
...
static void hookphuzz_context_begin(zend_execute_data *execute_data)
{
    ...
    hookphuzz_context_frame *parent = HOOKPHUZZ_PHASE8_G(context_count) == 0 ? NULL
        : &HOOKPHUZZ_PHASE8_G(contexts)[HOOKPHUZZ_PHASE8_G(context_count) - 1];
    ...
    HOOKPHUZZ_PHASE8_G(contexts) = erealloc(HOOKPHUZZ_PHASE8_G(contexts),
        sizeof(hookphuzz_context_frame) * (HOOKPHUZZ_PHASE8_G(context_count) + 1));
    frame = &HOOKPHUZZ_PHASE8_G(contexts)[HOOKPHUZZ_PHASE8_G(context_count)++];
...
static void hookphuzz_context_end(zend_execute_data *execute_data)
{
    ...
    for (index = HOOKPHUZZ_PHASE8_G(context_count); index > 0; index--) {
        ...
        HOOKPHUZZ_PHASE8_G(context_count)--;
        if (index - 1 != HOOKPHUZZ_PHASE8_G(context_count)) {
            HOOKPHUZZ_PHASE8_G(contexts)[index - 1] =
                HOOKPHUZZ_PHASE8_G(contexts)[HOOKPHUZZ_PHASE8_G(context_count)];
        }
        return;
    }
}
...
static void hookphuzz_release_events(void)
{
    ...
    for (index = 0; index < HOOKPHUZZ_PHASE8_G(event_count); index++) {
        hookphuzz_event *event = &HOOKPHUZZ_PHASE8_G(events)[index];
    ...
    if (HOOKPHUZZ_PHASE8_G(events) != NULL) efree(HOOKPHUZZ_PHASE8_G(events));
    HOOKPHUZZ_PHASE8_G(events) = NULL;
    HOOKPHUZZ_PHASE8_G(event_count) = 0;
}
...
static zend_observer_fcall_handlers hookphuzz_observer_init(zend_execute_data *execute_data)
{
    ...
    observe = hookphuzz_is_target(name) || HOOKPHUZZ_PHASE8_G(context_count) > 0;
...
static void hookphuzz_add_target_loading(zval *document)
{
    zval loading;
    array_init(&loading);
    add_assoc_long(&loading, "static_target_count", HOOKPHUZZ_PHASE8_G(static_target_count));
    add_assoc_long(&loading, "file_target_count", HOOKPHUZZ_PHASE8_G(file_target_count));
    add_assoc_long(&loading, "effective_target_count", HOOKPHUZZ_PHASE8_G(target_callback_count));
    add_assoc_string(&loading, "load_status", HOOKPHUZZ_PHASE8_G(target_load_status) == NULL ? "disabled"
        : ZSTR_VAL(HOOKPHUZZ_PHASE8_G(target_load_status)));
    if (HOOKPHUZZ_PHASE8_G(registry_schema_version) > 0) {
        add_assoc_long(&loading, "registry_schema_version", HOOKPHUZZ_PHASE8_G(registry_schema_version));
    } else add_assoc_null(&loading, "registry_schema_version");
    add_assoc_long(&loading, "duplicate_count", HOOKPHUZZ_PHASE8_G(target_duplicate_count));
    add_assoc_long(&loading, "rejected_count", HOOKPHUZZ_PHASE8_G(target_rejected_count));
...
static zend_result hookphuzz_encode_artifact(smart_str *json)
{
    ...
    array_init(&document);
    add_assoc_long(&document, "schema_version", 3);
...
static void hookphuzz_flush_artifact(void)
{
    ...
    if (hookphuzz_encode_artifact(&json) != SUCCESS) {
        hookphuzz_log("hookphuzz_opcode_phase8: artifact JSON encoding failed");
        return;
    }
    ...
    if (fd < 0 || hookphuzz_write_all(fd, ZSTR_VAL(json.s), ZSTR_LEN(json.s)) != SUCCESS || fsync(fd) != 0 || close(fd) != 0) {
        ...
        if (fd >= 0) close(fd);
        unlink(temp_path);
        snprintf(message, sizeof(message), "hookphuzz_opcode_phase8: artifact write failed for %s: %s",
    ...
    if (syscall(SYS_renameat2, AT_FDCWD, temp_path, AT_FDCWD, final_path, RENAME_NOREPLACE) != 0) {
        int error_code = errno;
        unlink(temp_path);
        snprintf(message, sizeof(message), "hookphuzz_opcode_phase8: artifact finalization failed for %s: %s",
            ZSTR_VAL(HOOKPHUZZ_PHASE5_G(request_id)), strerror(error_code));
        hookphuzz_log(message);
    }
    smart_str_free(&json);
...
PHP_MINIT_FUNCTION(hookphuzz_opcode_phase8)
{
...
PHP_MSHUTDOWN_FUNCTION(hookphuzz_opcode_phase8)
{
...
PHP_RINIT_FUNCTION(hookphuzz_opcode_phase8)
{
    zend_string *header;

#if defined(ZTS) && defined(COMPILE_DL_HOOKPHUZZ_OPCODE_PHASE8)
    ...
    HOOKPHUZZ_PHASE8_G(target_callbacks) = NULL;
    HOOKPHUZZ_PHASE8_G(target_callback_count) = 0;
    HOOKPHUZZ_PHASE8_G(static_target_count) = 0;
    HOOKPHUZZ_PHASE8_G(file_target_count) = 0;
    HOOKPHUZZ_PHASE8_G(target_duplicate_count) = 0;
    HOOKPHUZZ_PHASE8_G(target_rejected_count) = 0;
    HOOKPHUZZ_PHASE8_G(registry_schema_version) = 0;
    HOOKPHUZZ_PHASE8_G(target_load_status) = NULL;
    HOOKPHUZZ_PHASE8_G(file_targets_loaded) = 1;
    hookphuzz_parse_targets();
    hookphuzz_load_file_targets();

    header = hookphuzz_request_id_from_server();
    if (header == NULL) {
        hookphuzz_log("hookphuzz_opcode_phase8: request artifact skipped: missing X-Fuzzer-Covid");
        return SUCCESS;
    }
    if (!hookphuzz_valid_request_id(header)) {
        hookphuzz_log("hookphuzz_opcode_phase8: request artifact skipped: invalid X-Fuzzer-Covid");
...
PHP_RSHUTDOWN_FUNCTION(hookphuzz_opcode_phase8)
{
...
PHP_MINFO_FUNCTION(hookphuzz_opcode_phase8)
{
    php_info_print_table_start();
    php_info_print_table_header(2, "hookphuzz_opcode_phase8 support", "enabled");
...
zend_module_entry hookphuzz_opcode_phase8_module_entry = {
    STANDARD_MODULE_HEADER, "hookphuzz_opcode_phase8", NULL,
    PHP_MINIT(hookphuzz_opcode_phase8), PHP_MSHUTDOWN(hookphuzz_opcode_phase8),
    PHP_RINIT(hookphuzz_opcode_phase8), PHP_RSHUTDOWN(hookphuzz_opcode_phase8),
    PHP_MINFO(hookphuzz_opcode_phase8), PHP_HOOKPHUZZ_OPCODE_PHASE8_VERSION, STANDARD_MODULE_PROPERTIES
};

#ifdef COMPILE_DL_HOOKPHUZZ_OPCODE_PHASE8
# ifdef ZTS
ZEND_TSRMLS_CACHE_DEFINE();
# endif
ZEND_GET_MODULE(hookphuzz_opcode_phase8)
#endif
