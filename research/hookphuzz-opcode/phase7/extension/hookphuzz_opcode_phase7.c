...
#include "php.h"
#include "php_hookphuzz_opcode_phase7.h"
...
ZEND_DECLARE_MODULE_GLOBALS(hookphuzz_opcode_phase7)

PHP_INI_BEGIN()
    STD_PHP_INI_ENTRY("hookphuzz_opcode.target_callbacks", "", PHP_INI_SYSTEM, OnUpdateString,
        target_callbacks_ini, zend_hookphuzz_opcode_phase7_globals, hookphuzz_opcode_phase7_globals)
PHP_INI_END()
...
static void hookphuzz_release_targets(void)
{
    ...
    for (index = 0; index < HOOKPHUZZ_PHASE7_G(target_callback_count); index++) {
        zend_string_release(HOOKPHUZZ_PHASE7_G(target_callbacks)[index]);
    }
    if (HOOKPHUZZ_PHASE7_G(target_callbacks) != NULL) efree(HOOKPHUZZ_PHASE7_G(target_callbacks));
    HOOKPHUZZ_PHASE7_G(target_callbacks) = NULL;
    HOOKPHUZZ_PHASE7_G(target_callback_count) = 0;
}

static void hookphuzz_parse_targets(void)
{
    const char *cursor = HOOKPHUZZ_PHASE7_G(target_callbacks_ini);
    ...
    while (*cursor != '\0') {
        ...
        if (end > start) {
            HOOKPHUZZ_PHASE7_G(target_callbacks) = erealloc(HOOKPHUZZ_PHASE7_G(target_callbacks),
                sizeof(zend_string *) * (HOOKPHUZZ_PHASE7_G(target_callback_count) + 1));
            HOOKPHUZZ_PHASE7_G(target_callbacks)[HOOKPHUZZ_PHASE7_G(target_callback_count)++] =
                zend_string_init(start, end - start, 0);
        }
    }
}
...
static zend_bool hookphuzz_is_target(const zend_string *name)
{
    ...
    for (index = 0; index < HOOKPHUZZ_PHASE7_G(target_callback_count); index++) {
        if (zend_string_equals_ci(name, HOOKPHUZZ_PHASE7_G(target_callbacks)[index])) return 1;
    }
    return 0;
}
...
static hookphuzz_context_frame *hookphuzz_find_context(const zend_execute_data *execute_data)
{
    ...
    for (index = HOOKPHUZZ_PHASE7_G(context_count); index > 0; index--) {
        hookphuzz_context_frame *frame = &HOOKPHUZZ_PHASE7_G(contexts)[index - 1];
...
static void hookphuzz_release_contexts(void)
{
    ...
    for (index = 0; index < HOOKPHUZZ_PHASE7_G(context_count); index++) {
        zend_string_release(HOOKPHUZZ_PHASE7_G(contexts)[index].root_callback);
        zend_string_release(HOOKPHUZZ_PHASE7_G(contexts)[index].current_function);
    }
    if (HOOKPHUZZ_PHASE7_G(contexts) != NULL) efree(HOOKPHUZZ_PHASE7_G(contexts));
    HOOKPHUZZ_PHASE7_G(contexts) = NULL;
    HOOKPHUZZ_PHASE7_G(context_count) = 0;
}
...
static void hookphuzz_context_begin(zend_execute_data *execute_data)
{
    ...
    hookphuzz_context_frame *parent = HOOKPHUZZ_PHASE7_G(context_count) == 0 ? NULL
        : &HOOKPHUZZ_PHASE7_G(contexts)[HOOKPHUZZ_PHASE7_G(context_count) - 1];
    ...
    HOOKPHUZZ_PHASE7_G(contexts) = erealloc(HOOKPHUZZ_PHASE7_G(contexts),
        sizeof(hookphuzz_context_frame) * (HOOKPHUZZ_PHASE7_G(context_count) + 1));
    frame = &HOOKPHUZZ_PHASE7_G(contexts)[HOOKPHUZZ_PHASE7_G(context_count)++];
...
static void hookphuzz_context_end(zend_execute_data *execute_data)
{
    ...
    for (index = HOOKPHUZZ_PHASE7_G(context_count); index > 0; index--) {
        ...
        HOOKPHUZZ_PHASE7_G(context_count)--;
        if (index - 1 != HOOKPHUZZ_PHASE7_G(context_count)) {
            HOOKPHUZZ_PHASE7_G(contexts)[index - 1] =
                HOOKPHUZZ_PHASE7_G(contexts)[HOOKPHUZZ_PHASE7_G(context_count)];
        }
        return;
    }
}
...
static void hookphuzz_release_events(void)
{
    ...
    for (index = 0; index < HOOKPHUZZ_PHASE7_G(event_count); index++) {
        hookphuzz_event *event = &HOOKPHUZZ_PHASE7_G(events)[index];
    ...
    if (HOOKPHUZZ_PHASE7_G(events) != NULL) efree(HOOKPHUZZ_PHASE7_G(events));
    HOOKPHUZZ_PHASE7_G(events) = NULL;
    HOOKPHUZZ_PHASE7_G(event_count) = 0;
}
...
static zend_observer_fcall_handlers hookphuzz_observer_init(zend_execute_data *execute_data)
{
    ...
    observe = hookphuzz_is_target(name) || HOOKPHUZZ_PHASE7_G(context_count) > 0;
...
static zend_result hookphuzz_encode_artifact(smart_str *json)
{
    ...
    array_init(&document);
    add_assoc_long(&document, "schema_version", 2);
...
static void hookphuzz_flush_artifact(void)
{
    ...
    if (hookphuzz_encode_artifact(&json) != SUCCESS) {
        hookphuzz_log("hookphuzz_opcode_phase7: artifact JSON encoding failed");
        return;
    }
    ...
    if (fd < 0 || hookphuzz_write_all(fd, ZSTR_VAL(json.s), ZSTR_LEN(json.s)) != SUCCESS || fsync(fd) != 0 || close(fd) != 0) {
        ...
        if (fd >= 0) close(fd);
        unlink(temp_path);
        snprintf(message, sizeof(message), "hookphuzz_opcode_phase7: artifact write failed for %s: %s",
    ...
    if (syscall(SYS_renameat2, AT_FDCWD, temp_path, AT_FDCWD, final_path, RENAME_NOREPLACE) != 0) {
        int error_code = errno;
        unlink(temp_path);
        snprintf(message, sizeof(message), "hookphuzz_opcode_phase7: artifact finalization failed for %s: %s",
            ZSTR_VAL(HOOKPHUZZ_PHASE5_G(request_id)), strerror(error_code));
        hookphuzz_log(message);
    }
    smart_str_free(&json);
...
PHP_MINIT_FUNCTION(hookphuzz_opcode_phase7)
{
...
PHP_MSHUTDOWN_FUNCTION(hookphuzz_opcode_phase7)
{
...
PHP_RINIT_FUNCTION(hookphuzz_opcode_phase7)
{
    zend_string *header;

#if defined(ZTS) && defined(COMPILE_DL_HOOKPHUZZ_OPCODE_PHASE7)
    ...
    if (header == NULL) {
        hookphuzz_log("hookphuzz_opcode_phase7: request artifact skipped: missing X-Fuzzer-Covid");
        return SUCCESS;
    }
    if (!hookphuzz_valid_request_id(header)) {
        hookphuzz_log("hookphuzz_opcode_phase7: request artifact skipped: invalid X-Fuzzer-Covid");
...
PHP_RSHUTDOWN_FUNCTION(hookphuzz_opcode_phase7)
{
...
PHP_MINFO_FUNCTION(hookphuzz_opcode_phase7)
{
    php_info_print_table_start();
    php_info_print_table_header(2, "hookphuzz_opcode_phase7 support", "enabled");
...
zend_module_entry hookphuzz_opcode_phase7_module_entry = {
    STANDARD_MODULE_HEADER, "hookphuzz_opcode_phase7", NULL,
    PHP_MINIT(hookphuzz_opcode_phase7), PHP_MSHUTDOWN(hookphuzz_opcode_phase7),
    PHP_RINIT(hookphuzz_opcode_phase7), PHP_RSHUTDOWN(hookphuzz_opcode_phase7),
    PHP_MINFO(hookphuzz_opcode_phase7), PHP_HOOKPHUZZ_OPCODE_PHASE7_VERSION, STANDARD_MODULE_PROPERTIES
};

#ifdef COMPILE_DL_HOOKPHUZZ_OPCODE_PHASE7
# ifdef ZTS
ZEND_TSRMLS_CACHE_DEFINE();
# endif
ZEND_GET_MODULE(hookphuzz_opcode_phase7)
#endif
